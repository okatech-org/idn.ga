 ...
 * 
 * Ce module permet à iDN de BOUGER dans l'interface:
...
class MotorSynapseClass {
    ...
    private speechCompleteCallback: (() => void) | null = null;

    private constructor() {
        this.state = this.createInitialState();
        console.log('🦾 [MotorSynapse] Système moteur iDN initialisé');
    }
    ...
    public notifySpeechComplete(): void {
        ...
        this.notifyStateChange();
        if (this.speechCompleteCallback) {
            this.speechCompleteCallback();
            this.speechCompleteCallback = null;
        }
    }
    ...
    public welcomeSequence(): void {
        this.executeSequence({
            ...
            commands: [
                ...
                { type: 'VOCALIZE', text: 'Bienvenue sur IDN.ga !', emotion: 'happy', interruptible: false },
    ...
    /**
     * Animation de démarche administrative
     */
    public procedureSequence(procedureName: string): void {
        this.executeSequence({
            id: `procedure-${Date.now()}`,
            name: `Démarche: ${procedureName}`,
            commands: [
                { type: 'THINK', duration: 500, intensity: 'light' },
                { type: 'PULSE', intensity: 'medium', duration: 400 },
                { type: 'VOCALIZE', text: `Je vous guide pour ${procedureName}.`, emotion: 'neutral', interruptible: true }
            ]
        });
    }
    ...
    /**
     * Animation de vérification d'identité (pour contrôleurs)
     */
    public verificationSequence(): void {
        this.executeSequence({
            id: 'verification',
            name: 'Vérification d\'identité',
            commands: [
                { type: 'PULSE', intensity: 'medium', duration: 300 },
                { type: 'THINK', duration: 1000, intensity: 'deep' },
                { type: 'PULSE', intensity: 'strong', duration: 200 },
                { type: 'VOCALIZE', text: 'Analyse biométrique en cours...', emotion: 'neutral', interruptible: false }
            ]
        });
    }
    ...
    public reset(): void {
        ...
        this.isProcessing = false;
        this.notifyStateChange();
        console.log('🔄 [MotorSynapse] État réinitialisé');
    }
}
