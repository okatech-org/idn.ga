    ...
    return (
        <UserSpaceLayout>
            <div className="h-full flex flex-col gap-3">
                {/* Header */}
                <h1 className="text-lg font-bold text-foreground shrink-0">Paramètres</h1>
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <h2 className="font-bold text-sm text-foreground">Jean Dupont</h2>
                                <p className="text-[10px] text-muted-foreground truncate">jean.dupont@example.com</p>
                            </div>
                            <button className="text-[10px] text-primary font-medium">Modifier</button>
                        </motion.div>
                                ...
                                <div key={app.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 group">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[9px] text-muted-foreground">{app.permissions.join(", ")}</p>
                                        </div>
                                    </div>
                                    <button
...
const SettingsGroup = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div>
        <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wide mb-1.5 px-1">{title}</p>
        <div className={cn(
...
const SettingsItem = ({ icon: Icon, title, value, onClick }: { icon: React.ElementType; title: string; value?: string; onClick?: () => void }) => (
    <div onClick={onClick} className="flex items-center justify-between p-2 rounded-lg cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 group">
        <div className="flex items-center gap-2">
            <Icon className="w-4 h-4 text-muted-foreground group-hover:text-primary" />
            <span className="text-xs font-medium text-foreground">{title}</span>
        </div>
        <div className="flex items-center gap-1">
            {value && <span className="text-[10px] text-muted-foreground">{value}</span>}
...
const SettingToggle = ({ icon: Icon, title, checked, onCheckedChange }: { icon: React.ElementType; title: string; checked: boolean; onCheckedChange: (c: boolean) => void }) => (
    <div className="flex items-center justify-between p-2 rounded-lg">
        ...
        <button
            ...
            className={cn("w-9 h-5 rounded-full transition-colors relative", checked ? "bg-primary" : "bg-slate-200 dark:bg-slate-700")}
        >
